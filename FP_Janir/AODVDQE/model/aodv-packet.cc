...
RreqHeader::RreqHeader(uint8_t flags,
    ...
    : m_flags(flags),
      ...
      // --- TAMBAHAN AODV-DQE: Inisialisasi ---
      m_energy(1.0),   // Default: Baterai Penuh
      m_pathCost(0.0)  // Default: Cost 0
      // ---------------------------------------
{
}
...
{
    // tambahan 48 byte untuk Posisi(24) dan Kecepatan(24)
    return 23 + 48 + 16;
}
...
RreqHeader::Serialize(Buffer::Iterator i) const
{
    ...
    // Posisi
    std::memcpy(&buffer, &m_position.x, sizeof(double)); i.WriteHtonU64(buffer);
    std::memcpy(&buffer, &m_position.y, sizeof(double)); i.WriteHtonU64(buffer);
    std::memcpy(&buffer, &m_position.z, sizeof(double)); i.WriteHtonU64(buffer);
    
    // Kecepatan
    std::memcpy(&buffer, &m_velocity.x, sizeof(double)); i.WriteHtonU64(buffer);
    std::memcpy(&buffer, &m_velocity.y, sizeof(double)); i.WriteHtonU64(buffer);
    std::memcpy(&buffer, &m_velocity.z, sizeof(double)); i.WriteHtonU64(buffer);

    //tambahan QE
    // Energi (BARU)
    std::memcpy(&buffer, &m_energy, sizeof(double)); i.WriteHtonU64(buffer);
    // Path Cost (BARU)
    std::memcpy(&buffer, &m_pathCost, sizeof(double)); i.WriteHtonU64(buffer);
    // -------------------------------------
    // -----------------------
}
...
RreqHeader::Deserialize(Buffer::Iterator start)
{
    ...
    // Baca Posisi
    buffer = i.ReadNtohU64(); std::memcpy(&m_position.x, &buffer, sizeof(double));
    buffer = i.ReadNtohU64(); std::memcpy(&m_position.y, &buffer, sizeof(double));
    buffer = i.ReadNtohU64(); std::memcpy(&m_position.z, &buffer, sizeof(double));

    // Baca Kecepatan
    buffer = i.ReadNtohU64(); std::memcpy(&m_velocity.x, &buffer, sizeof(double));
    buffer = i.ReadNtohU64(); std::memcpy(&m_velocity.y, &buffer, sizeof(double));
    buffer = i.ReadNtohU64(); std::memcpy(&m_velocity.z, &buffer, sizeof(double));
    // -----------------------

    //tambahan QE
    // Energi (BARU)
    buffer = i.ReadNtohU64(); std::memcpy(&m_energy, &buffer, sizeof(double));   
    // Path Cost (BARU)
    buffer = i.ReadNtohU64(); std::memcpy(&m_pathCost, &buffer, sizeof(double));
    // ------------------------------------
...
//tambahan 
// --- TAMBAHAN AODV-D IMPLEMENTASI ---
void RreqHeader::SetPosition(Vector p) { m_position = p; }
Vector RreqHeader::GetPosition() const { return m_position; }
void RreqHeader::SetVelocity(Vector v) { m_velocity = v; }
Vector RreqHeader::GetVelocity() const { return m_velocity; }

// --- TAMBAHAN AODV-DQE ---
void RreqHeader::SetEnergy(double e) { m_energy = e; }
double RreqHeader::GetEnergy() const { return m_energy; }
void RreqHeader::SetPathCost(double c) { m_pathCost = c; }
double RreqHeader::GetPathCost() const { return m_pathCost; }
// -------------------------
// ------------------------------------
...
RrepHeader::Serialize(Buffer::Iterator i) const
{
    ...
    //tambahan
    // --- TAMBAHAN AODV-D (YANG LUPA DITAMBAHKAN) ---
    uint64_t buffer;
...
//tambahan
// --- TAMBAHAN AODV-D UNTUK RREP ---
void RrepHeader::SetPosition(Vector p) { m_position = p; }
Vector RrepHeader::GetPosition() const { return m_position; }
void RrepHeader::SetVelocity(Vector v) { m_velocity = v; }
Vector RrepHeader::GetVelocity() const { return m_velocity; }

// --- TAMBAHAN AODV-DQE ---
void RrepHeader::SetEnergy(double e) { m_energy = e; }
double RrepHeader::GetEnergy() const { return m_energy; }
void RrepHeader::SetPathCost(double c) { m_pathCost = c; }
double RrepHeader::GetPathCost() const { return m_pathCost; }
// -------------------------
// ----------------------------------
