...
//tambahan QE
// --- TAMBAHAN AODV-DQE: Include Energy ---
...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  ...
  private:
    ...
    //tambahan
    // --- TAMBAHAN AODV-D STEP 2: Variabel & Fungsi Helper ---
    
    // Parameter Bobot untuk Rumus (1) Link Weight
    double m_ws; // Speed Weight Factor
    double m_wd; // Direction Weight Factor
    
    // Pointer ke model mobilitas (agar node tahu posisi dirinya sendiri)
    Ptr<MobilityModel> m_mobility; 

    // Fungsi bantu untuk mengambil data fisik node saat ini
    Vector GetPosition () const;
    Vector GetVelocity () const;
    
    // Fungsi Matematika sesuai Paper
    // Hitung Link Weight (Persamaan 1)
    double CalculateLinkWeight (Vector myPos, Vector myVel, Vector neighborPos, Vector neighborVel);
    
    // Hitung Link Expiry Time / LET (Persamaan 2 & Gambar 2)
    Time CalculateLET (Vector myPos, Vector myVel, Vector neighborPos, Vector neighborVel);

    // ==============================================================
    //     MULAI TAMBAHAN UNTUK AODV-DQE (QoS & ENERGY) DI SINI
    // ==============================================================

    // 1. Variabel Bobot Baru (diatur via Attribute nantinya)
    double m_we; // Weight factor for Energy
    double m_wq; // Weight factor for QoS (Delay)

    // 2. Pointer ke Sumber Energi Node (Baterai)
    Ptr<ns3::energy::BasicEnergySource> m_energySource;

    // 3. Fungsi Bantu: Cek Sisa Energi (Return 0.0 s/d 1.0)
    double GetResidualEnergyLevel();

    // 4. Rumus COST AODV-DQE (Gabungan Mobilitas + Energi + QoS)
    //    Ini akan menggantikan peran CalculateLinkWeight yang lama
    double CalculateLinkCost(Vector myPos, Vector myVel, Vector neiPos, Vector neiVel, double neiEnergy);
