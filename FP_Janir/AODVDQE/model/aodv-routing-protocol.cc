...
// tambahan
#include "ns3/double.h" // <--- WAJIB DITAMBAHKAN AGAR DoubleValue DIKENALI
    ...
    namespace aodv
    {
        ...
        RoutingProtocol::RoutingProtocol()
            : m_rreqRetries(2),
              ...
              // tambahan
              m_ws(1.0), // Inisialisasi Bobot Kecepatan
              m_wd(1.0), // Inisialisasi Bobot Arah
        ...
        RoutingProtocol::GetTypeId()
        {
            static TypeId tid =
                TypeId("ns3::aodv::RoutingProtocol")
                    ...
                    // --- TAMBAHAN AODV-D: PERHATIKAN TIDAK ADA TITIK KOMA DI ATAS ---
                    .AddAttribute("Ws", "Weight factor for Speed",
                                  DoubleValue(1.0),
                                  MakeDoubleAccessor(&RoutingProtocol::m_ws),
                                  MakeDoubleChecker<double>())
                    .AddAttribute("Wd", "Weight factor for Direction",
                                  DoubleValue(1.0),
                                  MakeDoubleAccessor(&RoutingProtocol::m_wd),
                                  MakeDoubleChecker<double>())

                    // --- TAMBAHAN AODV-DQE ---
                    .AddAttribute("We", "Weight factor for Energy",
                                  DoubleValue(1.0),
                                  MakeDoubleAccessor(&RoutingProtocol::m_we),
                                  MakeDoubleChecker<double>())
                    .AddAttribute("Wq", "Weight factor for QoS",
                                  DoubleValue(1.0),
                                  MakeDoubleAccessor(&RoutingProtocol::m_wq),
                                  MakeDoubleChecker<double>());
            // -------------------------
            // ----------------------------------------------------------------
        ...
        RoutingProtocol::Start()
        {
            ...
            // tambahan
            //  --- TAMBAHAN AODV-D ---
            //  Ambil akses ke posisi node saat ini
        ...
        RoutingProtocol::SendRequest(Ipv4Address dst)
        {
            ...
            // tambahan
            //  --- TAMBAHAN AODV-D ---
            rreqHeader.SetPosition(GetPosition());
            rreqHeader.SetVelocity(GetVelocity());

            // Set Energi saya saat ini
            rreqHeader.SetEnergy(GetResidualEnergyLevel());

            // Set Cost awal = 0 (karena ini sumber)
            rreqHeader.SetPathCost(0.0);
            // -----------------------
        ...
        RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
        {
            ...
            //     // tambahan
            //     //  --- TAMBAHAN AODV-D: Node Selection Logic ---
            //     Vector senderPos = rreqHeader.GetPosition();
            //     Vector senderVel = rreqHeader.GetVelocity();
            //     Vector myPos = GetPosition();
            //     Vector myVel = GetVelocity();

            //     double linkWeight = CalculateLinkWeight(myPos, myVel, senderPos, senderVel);

            //     // Threshold Stabilitas (Bisa disesuaikan)
            //     double threshold = 1.5;

            //     if (linkWeight > threshold)
            //     {
            //         // --- TAMBAHAN LOG SAAT DROP ---
            //         NS_LOG_DEBUG("[AODV-D CHECK] >>> DROP RREQ from " << src << " because Link Weight (" << linkWeight << ") > Threshold!");
            //         // ------------------------------
            //         return; // Drop packet
            //     }
            ...
            //     if (linkWeight > threshold)
            //     {
            //         NS_LOG_DEBUG("AODV-D: Drop RREQ from " << src << " due to High Link Weight: " << linkWeight);
            //         return; // Drop packet
            //     }
            //     // ---------------------------------------------

            //     // --- TAMBAHAN LOG UNTUK DEBUGGING ---
            //     NS_LOG_DEBUG("[AODV-D CHECK] RREQ from " << src << ". Link Weight: " << linkWeight << " (Threshold: " << threshold << ")");
            //     // ------------------------------------
            ...
            // ==============================================================
            // [MULAI] MODIFIKASI AODV-DQE: Hitung Cost & Energi
            // ==============================================================

            // 1. Ambil data dari header paket yang diterima
            Vector senderPos = rreqHeader.GetPosition();
            Vector senderVel = rreqHeader.GetVelocity();
            double senderEnergy = rreqHeader.GetEnergy();       // Energi sisa pengirim
            double incomingPathCost = rreqHeader.GetPathCost(); // Cost dari Source sampai Node Pengirim

            // 2. Hitung Cost untuk Link ini saja (Node Pengirim <-> Saya)
            //    Menggunakan fungsi CalculateLinkCost yang sudah kita buat sebelumnya
            double linkCost = CalculateLinkCost(GetPosition(), GetVelocity(), senderPos, senderVel, senderEnergy);

            // 3. Hitung Total Cost Rute Baru (Accumulated Metric)
            //    Total = Cost sebelumnya + Cost link baru
            double newTotalCost = incomingPathCost + linkCost;

            NS_LOG_DEBUG("AODV-DQE: RREQ from " << src << " (Origin: " << rreqHeader.GetOrigin() << ")");
            NS_LOG_DEBUG("   - Incoming Cost: " << incomingPathCost);
            NS_LOG_DEBUG("   - Link Cost: " << linkCost << " (Energy: " << senderEnergy << ")");
            NS_LOG_DEBUG("   - New Total Cost: " << newTotalCost);

            // ==============================================================
            // [SELESAI] MODIFIKASI AWAL
            // ==============================================================
            ...
            if (!m_routingTable.LookupRoute(origin, toOrigin))
            {
                // Rute belum ada, buat entri baru
                Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
                RoutingTableEntry newEntry(
                    /*dev=*/dev,
                    /*dst=*/origin,
                ...
                // [AODV-DQE] Simpan Cost Baru ke Tabel Routing
                newEntry.SetPathCost(newTotalCost);

                m_routingTable.AddRoute(newEntry);
            }
            else
            {
                // Rute sudah ada, cek apakah perlu di-update?
                bool updateRoute = false;

                if (toOrigin.GetValidSeqNo())
                {
                    // Kondisi 1: Sequence Number Pengirim LEBIH BARU (Prioritas Utama)
                    if (int32_t(rreqHeader.GetOriginSeqno()) - int32_t(toOrigin.GetSeqNo()) > 0)
                    {
                        updateRoute = true;
                    }
                    // Kondisi 2: Sequence Number SAMA, tapi...
                    else if (int32_t(rreqHeader.GetOriginSeqno()) == int32_t(toOrigin.GetSeqNo()))
                    {
                        // [AODV-DQE LOGIC]
                        // Update jika Cost Baru LEBIH KECIL dari Cost Lama (Lebih Efisien)
                        // AODV Standar pakai 'hop < toOrigin.GetHop()', kita ganti dengan Cost.
                        if (newTotalCost < toOrigin.GetPathCost())
                        {
                            updateRoute = true;
                            NS_LOG_DEBUG("AODV-DQE: Better path found! (NewCost: " << newTotalCost << " < OldCost: " << toOrigin.GetPathCost() << ")");
                        }
                    }
                }
                else
                {
                    // Sequence number lama invalid, update saja
                    updateRoute = true;
                }
                ...
                if (updateRoute)
                {
                    ...
                    // [AODV-DQE] Update Cost di Tabel Routing
                    toOrigin.SetPathCost(newTotalCost);
            ...
            if (!m_routingTable.LookupRoute(src, toNeighbor))
            {
                ...
                // [AODV-DQE] Tetangga langsung cost-nya = link cost
                newEntry.SetPathCost(linkCost);
            ...
            else
            {
                ...
                // [AODV-DQE] Update cost tetangga
                toNeighbor.SetPathCost(linkCost);
            ...
            // ==============================================================
            // [MULAI] MODIFIKASI AODV-DQE: Update Header Sebelum Forward
            // ==============================================================
            // Sebelum paket ini di-broadcast ulang ke tetangga, kita harus
            // memperbarui isinya dengan data node kita.

            // 1. Update Cost: Akumulasi Cost sampai titik ini (Sumber -> Saya)
            rreqHeader.SetPathCost(newTotalCost);

            // 2. Update Energi: Beritahu tetangga berikutnya status energi SAYA
            rreqHeader.SetEnergy(GetResidualEnergyLevel());

            // 3. Update Mobilitas: Beritahu tetangga berikutnya posisi & kecepatan SAYA
            rreqHeader.SetPosition(GetPosition());
            rreqHeader.SetVelocity(GetVelocity());

            // ==============================================================
            // [SELESAI] MODIFIKASI HEADER
            // ==============================================================
        ...
        RoutingProtocol::SendReply(const RreqHeader &rreqHeader, const RoutingTableEntry &toOrigin)
        {
            ...
            // tambahan
            //  --- TAMBAHAN AODV-D ---
            rrepHeader.SetPosition(GetPosition());
            rrepHeader.SetVelocity(GetVelocity());
            // -----------------------
                                                     ...
                                                     bool gratRep)
        {
            ...
            // tambahan QE
            rrepHeader.SetEnergy(GetResidualEnergyLevel());
            rrepHeader.SetPathCost(0.0); // Cost dimulai dari 0 lagi dari sisi Destination
            // -----------------------
        ...
        RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
        {
            ...
            // // tambahan
            // //  --- TAMBAHAN AODV-D ---
            // Vector senderPos = rrepHeader.GetPosition();
            // Vector senderVel = rrepHeader.GetVelocity();

            // // 1. Hitung LET untuk Link lokal ini (Saya <-> Pengirim)
            // Time currentLinkLET = CalculateLET(GetPosition(), GetVelocity(), senderPos, senderVel);

            // // 2. Ambil Cumulative LET yang dibawa dari hop sebelumnya (dari Destination arah ke sini)
            // // Jika ini adalah hop pertama dari Destination, anggap tak hingga/sangat lama
            // Time pathLETSoFar = rrepHeader.GetLifeTime();

            // // --- PERBAIKAN: Gunakan 'currentLinkLET' ---
            // NS_LOG_DEBUG("[AODV-D CHECK] RREP from " << sender << ". Calculated LET: " << currentLinkLET.As(Time::S) << "s");
            // // -------------------------------------------

            // // 3. Logika MINIMUM (Link terlemah menentukan umur rute)
            // // Jika pathLETSoFar 0 (biasanya inisialisasi), kita pakai currentLinkLET
            // // Jika tidak, kita ambil yang paling kecil antara Link ini vs Sisa Path
            // Time finalLET;
            // if (pathLETSoFar == MilliSeconds(0) || currentLinkLET < pathLETSoFar)
            // {
            //     finalLET = currentLinkLET;
            // }
            // else
            // {
            //     finalLET = pathLETSoFar;
            // }

            // // 4. Update Header sebelum diteruskan ke node sebelumnya (menuju Source)
            // if (finalLET > Time(0))
            // {
            //     rrepHeader.SetLifeTime(finalLET);
            // }
            // // -----------------------
            ...
            //     // tambahan
            //     //  Logika Update AODV-D: Pilih rute jika SeqNo lebih baru ATAU (SeqNo sama TAPI Lifetime lebih lama)
            //     if (
            //         (!toDst.GetValidSeqNo()) ||
            //         ((int32_t(rrepHeader.GetDstSeqno()) - int32_t(toDst.GetSeqNo())) > 0) ||
            //         (rrepHeader.GetDstSeqno() == toDst.GetSeqNo() && toDst.GetFlag() != VALID) ||
            //         // --- PERUBAHAN UTAMA: Membandingkan Lifetime (Stabilitas) ---
            //         (rrepHeader.GetDstSeqno() == toDst.GetSeqNo() && rrepHeader.GetLifeTime() > toDst.GetLifeTime()))
            //     {
            //         m_routingTable.Update(newEntry);
            //     }
            // }
            ...
            // ==============================================================
            // [MULAI] MODIFIKASI AODV-DQE: Hitung Cost RREP
            // ==============================================================

            // 1. Ambil data dari header RREP
            Vector senderPos = rrepHeader.GetPosition();
            Vector senderVel = rrepHeader.GetVelocity();
            double senderEnergy = rrepHeader.GetEnergy();
            double incomingPathCost = rrepHeader.GetPathCost();

            // 2. Hitung Cost Link (Saya <-> Pengirim RREP)
            //    Gunakan fungsi CalculateLinkCost (Mobilitas + Energi + QoS)
            double linkCost = CalculateLinkCost(GetPosition(), GetVelocity(), senderPos, senderVel, senderEnergy);

            // 3. Hitung Total Cost Rute Balik (Accumulated Metric)
            double newTotalCost = incomingPathCost + linkCost;

            NS_LOG_DEBUG("AODV-DQE: RREP from " << sender << " (Dst: " << rrepHeader.GetDst() << ")");
            NS_LOG_DEBUG("   - Incoming Cost: " << incomingPathCost);
            NS_LOG_DEBUG("   - Link Cost: " << linkCost);
            NS_LOG_DEBUG("   - New Total Cost: " << newTotalCost);
            ...
            // [AODV-DQE] Simpan Cost Rute Baru
            newEntry.SetPathCost(newTotalCost);
            ...
            {
                // ==============================================================
                // [MULAI] MODIFIKASI AODV-DQE: Logika Dijkstra (Update Table)
                // ==============================================================
                /*
                Logika Update AODV Standar:
                Update jika (SeqNo Baru > Lama) ATAU (SeqNo Sama DAN Hop Baru < Lama)

                Logika Update AODV-DQE:
                Update jika (SeqNo Baru > Lama) ATAU (SeqNo Sama DAN Cost Baru < Lama)
                */

                if (
                    // 1. Sequence Number lebih baru (Wajib diterima karena info paling update)
                    (!toDst.GetValidSeqNo()) ||
                    ((int32_t(rrepHeader.GetDstSeqno()) - int32_t(toDst.GetSeqNo())) > 0) ||

                    // 2. Sequence Number sama, tapi Rute Invalid
                    (rrepHeader.GetDstSeqno() == toDst.GetSeqNo() && toDst.GetFlag() != VALID) ||

                    // 3. Sequence Number sama, tapi COST LEBIH KECIL (Lebih Efisien/Stabil)
                    //    (Menggantikan logika 'hop < toDst.GetHop()')
                    (rrepHeader.GetDstSeqno() == toDst.GetSeqNo() && newTotalCost < toDst.GetPathCost()))
                {
                    // Update rute dengan entry baru (yang sudah berisi cost baru)
                    m_routingTable.Update(newEntry);
                    NS_LOG_DEBUG("AODV-DQE: Updated Route to " << dst << " with better Cost: " << newTotalCost);
                }
                // ==============================================================
                // [SELESAI] MODIFIKASI LOGIKA
                // ==============================================================
            }
            else
            {
            ...
            // Jika saya adalah Originator (Pembuat RREQ awal), maka proses selesai.
            if (IsMyOwnAddress(rrepHeader.GetOrigin()))
            {
                if (toDst.GetFlag() == IN_SEARCH)
                {
            ...
            // Jika saya bukan Originator, saya harus meneruskan (Forward) RREP ini ke hop berikutnya.
            RoutingTableEntry toOrigin;
            if (!m_routingTable.LookupRoute(rrepHeader.GetOrigin(), toOrigin) ||
                toOrigin.GetFlag() == IN_SEARCH)
            {
            ...
            // [MULAI] MODIFIKASI AODV-DQE: Update Header Sebelum Forward
            // ==============================================================
            // Sebelum meneruskan RREP ke node berikutnya (menuju Source),
            // kita update data cost dan status node kita saat ini.

            // 1. Update Accumulated Cost (Total biaya dari Destination sampai sini)
            rrepHeader.SetPathCost(newTotalCost);

            // 2. Update Energi SAYA
            rrepHeader.SetEnergy(GetResidualEnergyLevel());

            // 3. Update Posisi & Kecepatan SAYA
            rrepHeader.SetPosition(GetPosition());
            rrepHeader.SetVelocity(GetVelocity());
        ...
        // tambahan
        //  --- TAMBAHAN AODV-D: Helper Functions (RUMUS MATEMATIKA) ---
        Vector
        ...
        Vector
        RoutingProtocol::GetVelocity() const
        {
            if (m_mobility)
        ...
        double
        RoutingProtocol::CalculateLinkWeight(Vector myPos, Vector myVel, Vector neiPos, Vector neiVel)
        {
            // Rumus (1) Paper: Link Weight
            double mySpeed = std::sqrt(myVel.x * myVel.x + myVel.y * myVel.y);
            double neiSpeed = std::sqrt(neiVel.x * neiVel.x + neiVel.y * neiVel.y);
            double speedDiff = std::abs(mySpeed - neiSpeed);

            double dirDiff = 0.0;
            if (mySpeed > 0 && neiSpeed > 0)
            {
                // Kosinus arah vector
                double dot = myVel.x * neiVel.x + myVel.y * neiVel.y;
                double mag = mySpeed * neiSpeed;
                dirDiff = 1.0 - (dot / mag); // Semakin beda arah, semakin besar nilainya
            }

            return (m_ws * speedDiff) + (m_wd * dirDiff);
        }

        Time
        RoutingProtocol::CalculateLET(Vector myPos, Vector myVel, Vector neiPos, Vector neiVel)
        {
            // Rumus Link Expiry Time (LET) - Prediksi kapan sinyal putus
            double R = 250.0; // Asumsi jangkauan sinyal Wifi (meter)

            double relVelX = myVel.x - neiVel.x;
            double relVelY = myVel.y - neiVel.y;
            double relPosX = myPos.x - neiPos.x;
            double relPosY = myPos.y - neiPos.y;

            double a = relVelX * relVelX + relVelY * relVelY;
            double b = 2 * (relPosX * relVelX + relPosY * relVelY);
            double c = (relPosX * relPosX + relPosY * relPosY) - (R * R);

            if (a == 0)
                return Seconds(1000); // Bergerak sejajar = koneksi lama
        ...
        // ============================================================================
        //  IMPLEMENTASI TAMBAHAN AODV-DQE (Copy bagian ini ke paling bawah file .cc)
        // ============================================================================

        double
        RoutingProtocol::GetResidualEnergyLevel()
        {
            // Cek apakah pointer ke sumber energi valid
            if (m_energySource != nullptr)
            {
                // Mengembalikan rasio: Sisa Energi / Energi Awal (Hasil 0.0 s.d 1.0)
                // Mengakses method public dari BasicEnergySource
                return m_energySource->GetRemainingEnergy() / m_energySource->GetInitialEnergy();
            }

            // Jika tidak ada model energi terpasang, anggap baterai penuh (1.0)
            return 1.0;
        }

        double
        RoutingProtocol::CalculateLinkCost(Vector myPos, Vector myVel, Vector neiPos, Vector neiVel, double neiEnergy)
        {
            // 1. Hitung Faktor Mobilitas
            //    Kita panggil fungsi CalculateLinkWeight yang sudah ada (Speed + Direction)
            double mobilityCost = CalculateLinkWeight(myPos, myVel, neiPos, neiVel);

            // 2. Hitung Faktor Energi (Inverse)
            //    Semakin kecil energi (neiEnergy mendekati 0), Cost harus semakin BESAR.
            //    Rumus: Cost = 1 / (Energi + epsilon)
            double energyCost = 0.0;
            if (neiEnergy > 0.001)
                energyCost = 1.0 / neiEnergy;
            else
                energyCost = 1000.0; // Penalti besar jika baterai sekarat (hampir 0)

            // 3. Hitung Faktor QoS (Estimasi Delay)
            //    Sebagai simplifikasi, kita pakai NodeTraversalTime (waktu proses per hop)
            double qosCost = m_nodeTraversalTime.GetSeconds();

            // --- RUMUS FINAL AODV-DQE ---
            // Total Cost = (Mobility) + (We * Energy) + (Wq * QoS)
            // Catatan: mobilityCost sudah mengandung bobot Ws dan Wd di dalamnya.

            double totalCost = mobilityCost + (m_we * energyCost) + (m_wq * qosCost);
