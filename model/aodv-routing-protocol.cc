...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    Time let = CalculateLET(GetPosition(), GetVelocity(), senderPos, senderVel);
    if (let > Time(0)) {
        rrepHeader.SetLifeTime(let);
    }
    // -----------------------
...
{
    if (m_mobility) return m_mobility->GetPosition();
    return Vector(0, 0, 0);
}
...
{
    if (m_mobility) return m_mobility->GetVelocity();
    return Vector(0, 0, 0);
}
...
RoutingProtocol::CalculateLET(Vector myPos, Vector myVel, Vector neiPos, Vector neiVel)
{
    ...
    if (a == 0) return Seconds(1000); // Bergerak sejajar = koneksi lama

    double discriminant = b * b - 4 * a * c;
    if (discriminant < 0) return Seconds(0); 
